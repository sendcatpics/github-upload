...
#Fix incorrect format of forwarding number
numberStr = re.sub("[^0-9]", "", str(numberStr))
formatCatch = numberStr.startswith('0')
if formatCatch is True:
    numberStr = numberStr.replace("0", "64", 1)
#Construct String
with open(fileStr,'r') as f:
    for line in f:
        line=re.sub("[^0-9]", "", str(line))
        formatCatch = line.startswith('0')
        if formatCatch is True:
            line=line.replace("0", "64", 1)
